...
export function GroupView({ groupId, serviceId, month, year, status }: GroupViewProps) {
  const [page, setPage] = useState(1)
  const [search, setSearch] = useState('')
  const [drawerOpen, setDrawerOpen] = useState(false)
  const [selectedDeadline, setSelectedDeadline] = useState<any>(null)

  const ready = !!groupId && !!serviceId
  ...
  if (!ready) {
    return <p className="text-muted-foreground text-center py-12">Select a client group and a service to view compliance status for group members.</p>
  }
  ...
  return (
    <>
      <DataGrid
        ...
        isLoading={isLoading}
        searchPlaceholder="Search group clients..."
        onSearch={setSearch}
